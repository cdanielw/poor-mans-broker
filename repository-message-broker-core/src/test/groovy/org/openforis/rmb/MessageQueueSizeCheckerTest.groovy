...
class MessageQueueSizeCheckerTest extends Specification {
    ...
    def checker = new MessageQueueSizeChecker(messageRepository, monitors)
    ...
    def 'Given a queue with no messages, when checking, monitor is notified'() {
        ...
        checker.includeQueue('queue id', [consumer])
        repositoryReturns([:])
    ...
    def 'Given a queue with two consumers with one and two messages, when checking, monitor is notified for each consumer'() {
        ...
        checker.includeQueue('queue id', [consumer1, consumer2])
    ...
    def 'Given two queues with a consumer each with one and two messages, when checking, monitor is notified for each consumer'() {
        def consumer1 = consumer('consumer 1')
        def consumer2 = consumer('consumer 2')
        checker.includeQueue('queue 1', [consumer1])
        checker.includeQueue('queue 2', [consumer2])
        repositoryReturns((consumer1): 1, (consumer2): 2)
        def events = [] as List<MessageQueueSizeChangedEvent>
    ...
    def 'When getting checking for queue size fails, monitors is passed a CheckingForMessageQueueSizeChangesFailedEvent'() {
        checker.includeQueue('queue id', [consumer('consumer id')])
        messageRepository.messageCountByConsumer(*_) >> { throw new RuntimeException('Repository failure') }

        when:
            checker.check()
        then:
            1 * monitor.onEvent(_ as CheckingForMessageQueueSizeChangesFailedEvent)
    }
